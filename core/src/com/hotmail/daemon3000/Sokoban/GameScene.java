...
public class GameScene implements Scene {
	...
	private EventListener<MoveDirection> m_moveListener;
	private ActionListener m_undoListener;
	private ActionListener m_pauseListener;
	...
	public void onLoad() {
		...
		m_moveListener = new EventListener<MoveDirection>() {
			@Override
			public void handle(MoveDirection arg) {
				m_gameplayScreen.movePlayer(arg);
			}
		};
		
		m_undoListener = new ActionListener() {
			@Override
			public void handle() {
				m_gameplayScreen.undoMovePlayer();
			}
		};
		
		m_pauseListener = new ActionListener() {
			@Override
			public void handle() {
				m_screenManager.pushScreen(ScreenID.Pause);
			}
		};
		
		m_inputAdapter = m_game.getPlatformAdapter().getInputAdapter();
		m_inputAdapter.addMoveListener(m_moveListener);
		m_inputAdapter.addUndoListener(m_undoListener);
		m_inputAdapter.addPauseListener(m_pauseListener);
	...
	public void update(float delta) {
		m_inputAdapter.update(Gdx.graphics.getDeltaTime());
	...
	public void dispose() {
		...
		m_uiSkin.dispose();
		m_click.dispose();
		m_inputAdapter.removeMoveListener(m_moveListener);
		m_inputAdapter.removeUndoListener(m_undoListener);
		m_inputAdapter.removePauseListener(m_pauseListener);
	}
}
